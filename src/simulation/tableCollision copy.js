...
const POCKET_EDGE_MARGIN = 0.012;
...
export const POCKET_CENTERS = POCKET_POSITIONS.map(([tx, tz]) => ({
  ...
  mouthRadius: BALL.radius * 4.5,
  edgeRadius: BALL.radius * 2.35,
}));

export function getPocketRegion(pocket) {
  return {
    center: { x: pocket.x, y: pocket.y },
    mouthRadius: pocket.mouthRadius + POCKET_EDGE_MARGIN,
    edgeRadius: pocket.edgeRadius + POCKET_EDGE_MARGIN,
  };
}
